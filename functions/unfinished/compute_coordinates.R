...
training_and_certification_propose_training$sequenceFlows -> edges
...
tibble(node_id = as.numeric(nodes %>% html_node("title") %>%  html_text()),
       x = as.numeric(nodes %>% html_node("ellipse") %>% html_attr("cx")),
       y = as.numeric(nodes %>% html_node("ellipse") %>% html_attr("cy"))) -> coordinates
